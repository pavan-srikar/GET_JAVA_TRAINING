...
SELECT productCode, productName, textDescription FROM products p
	INNER JOIN productlines p1
		ON p.productLine=p1.productLine;
		
SELECT productCode, productName, textDescription FROM products p
	INNER JOIN productlines
		ON products.productLine=products.productLine;
...
SELECT employeeNumber, lastName,firstName FROM employees  -- OUTER query
	WHERE officeCode
		IN(SELECT officeCode FROM offices WHERE country='USA');      -- inner query acts as critical value
